<template>
  <li v-if="canAccess(item.permission)" class="nav-item has-sub" :class="{ 'open': isOpen, 'disabled': item.disabled, 'sidebar-group-active': isActive, }">
    ...
    <b-collapse v-model="isOpen" class="menu-content" tag="ul">
      <component
        ...
        :key="$t(child.header) || $t(child.title)"
        ref="groupChild"
        :item="child"
		    :show="canAccess(child.permission)"
      />
    </b-collapse>
  </li>
</template>
