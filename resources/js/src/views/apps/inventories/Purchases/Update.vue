                        ...
                        <b-card no-body class="invoice-preview-card">           
                            ...
                            <b-card-body class="invoice-padding pt-0">
                                <b-row>
                                    ...
                                    <b-col cols="12" lg="2">
                                        <b-button @click="updatePurchase" v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="primary" class="" block>
                                            {{ $t('save') }}
                                        </b-button>
                                    </b-col>
                                </b-row>
...
export default {
    ...
    setup() {
        ...
        const getPurchaseDetail = async (id) => {
            loading.value = true
            const { data } = await PurchaseResource.show(id);
            loading.value = false
        ...
        onMounted(async () => {
            await getSuppliers()
            await getWarehouses()
            await getPurchaseDetail(router.currentRoute.params.id)
        })
        ...
        const updatePurchase = async () => {            
            ...
            try {
                loading.value = true
                const { data } = await PurchaseResource.update(router.currentRoute.params.id, invoiceData.value)
                loading.value = false
                if (data.success) {
                    toast({
                        component: ToastificationContent,
