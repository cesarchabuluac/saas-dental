<template>
    <b-overlay :show="loading" blur="2px" variant="transparent" rounded="lg" opacity="0.85">
        ...
        <b-alert variant="danger" :show="budget.length <= 0 && !loading">
            <h4 class="alert-heading">Error fetching budget data</h4>
            <div class="alert-body">
                No budget found with this budget id. Check
                <b-link class="alert-link" :to="{ name: 'apps-budgets-list' }">
                    Budget List
                </b-link>
                for other budget.
            </div>
                                        ...
                                        <b-row class="invoice-spacing">
                                            <b-col cols="12" xl="12" class="p-1">
                                                <p class="card-text mb-25">
                                                    <strong>{{ $t('patients.document_type') }}({{ budget.patient.document_type }}):</strong>
                                                    {{ budget.patient.rut }}
                                                </p>
                                                <p class="card-text mb-25">
                                                    <strong>{{ $t("patients.address") }}:</strong>
                                                    {{ budget.patient.address }}
                                                </p>
                                                <p class="card-text mb-25">
                                                    <strong>{{ $t("patients.phone") }}:</strong>
                                                    {{ budget.patient.phone }},{{ budget.patient.cellphone }}
                                                </p>
                                                <p class="card-text mb-25">
                                ...
                                <b-col cols="12" class="p-1">
                                    ...
                                    <b-form ref="form" class="_repeater-form">
                                        ...
                                        <b-row v-for="(item, index) in items" :id="item.id" :key="item.id" ref="row"
                                            class="mt-1">
                                            <!-- Item Name -->
                                            <b-col v-if="item.action_type === 'clinical'" md="3">
                                                <!-- Action type field -->
                                            ...
                                            <!-- Action -->
                                            <b-col :md="item.action_type === 'clinical' ? 3 : 6">
                        ...
                        <b-card-body v-if="items.length > 0" class="invoice-padding pb-0_">
                            <b-row>
                                <b-col cols="12" md="6" class="mt-md-0 mt-3">
                                    ...
                                    <b-form-textarea v-model="budget.comments" id="comments"
                                        :placeholder="$t('budgets.comments_placeholder')" rows="3" />
                                </b-col>

                                <!-- Col: Total -->
                                <b-col cols="12" md="6" class="mt-md-0 mt-3 d-flex justify-content-end">
                                    <div class="invoice-total-wrapper">
                                        ...
                                        <b-button v-if="canAccess('budgets.update') && budget_total > 0"
...
export default {
    ...
    methods: {
        async getBranch() {
            ...
            this.loading = false
            this.branchs = data
            this.selectedBranch = this.branchs.filter(b => b.id === this.budget.branch_office_id)[0]
        },
        async getBudget() {
            this.loading = true
            ...
            this.items = _.orderBy(this.budget.budget_actions, 'action_type', 'asc')
            this.nextTodoId = _.size(this.items)
        },
        ...
        async update() {
            ...
            this.budget.branch_office_id = 1;
            ...
            try {
                this.loading = true;
                const { data } = await BudgetResource.update(this.budget.id, this.budget);
                this.loading = false;
        ...
        async selectActionGroup(evt) {
            delete evt.selectedAction
            this.getActions(evt.action_group.id);
        },
        selectAction(evt) {
            console.log(evt)
