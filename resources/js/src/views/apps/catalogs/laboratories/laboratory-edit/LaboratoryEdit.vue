<template>
    <b-overlay :show="loading" blur="2px" variant="transparent" rounded="lg" opacity="0.85">
        ...
        <!-- Alert: No item found -->
        <b-alert variant="danger" :show="laboratory === undefined">
            <h4 class="alert-heading">Error fetching laboratory actions data</h4>
            <div class="alert-body">
                No laboratory found with this actions id. Check
                <b-link class="alert-link" :to="{ name: 'apps-actions-list' }">
        ...
        <b-card no-body class="mb-0">
            <div class="m-2">
                <b-form>
                        ...
                        <b-col cols="12" md="4">
                            <b-form-group :label="$t('laboratories.phone')" :description="$t('laboratories.phone_help')"
                                ...
                                <VuePhoneNumberInput :dark="isDarkMode()" dark-color="#283046" v-model="laboratory.phone"
                                    required :default-country-code="laboratory.phone_country" no-flags :translations="{
                                    ...
                                    :no-use-browser-locale="false" ies. @update="validPhone" />
                            </b-form-group>
                        </b-col>
                    </b-row>
                    ...
                    <!-- Action Buttons -->
                    <div class="mt-3">
                        <b-button v-if="canAccess('laboratories.update')" variant="primary"
...
export default {
    ...
    async mounted() {
        this.laboratory_id = router.currentRoute.params.id;
    ...
    methods: {
        ...
        async getLaboratory() {
            try {
                this.loading = true;
                const { data } = await LaboratoryResource.getLaboratoryDetail(this.laboratory_id);
                this.loading = false;
                this.laboratory = data.data;
                // this.laboratory.document_type = this.documentTypes.find(dt => dt.value === data.data.document_type)
                this.isValidPhone = this.laboratory.mobil ? this.laboratory.mobile.is_valid : false
                this.validateRut()
            } catch (e) {
                this.loading = false;
        ...
        async update() {
            ...
            try {
                this.loading = true;
                const { data } = await LaboratoryResource.update(this.laboratory_id, this.laboratory);
                this.loading = false;
