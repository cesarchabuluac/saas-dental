            ...
            <b-row class="invoice-preview">
                            ...
                            <div class="d-flex justify-content-between flex-md-row flex-column invoice-spacing mt-3">
                                ...
                                <div class="mt-md-0 mt-2">
                                    ...
                                    <div class="invoice-date-wrapper">
                                        <b-form-group :label="$t('status')" label-for="status" class="col-md-12">
                                            <v-select id="status" v-model="workorder.status" :options="status"
                            ...
                            <b-row class="invoice-spacing">
                                <b-col cols="12" xl="8" class="p-1">
                                    <b-card-text class="mb-0">
                                        <span
                                            class="font-weight-bold">{{ $t('work_orders.delivery_materials') }}:</span>
                                    </b-card-text>
                                ...
                                <b-col cols="12" xl="8" class="p-1">
                                    <b-card-text class="mb-0">
                                        <span
                                            class="font-weight-bold">{{ $t('work_orders.clinical_specification') }}:</span>
                                    </b-card-text>
                ...
                <b-col cols="12" md="4" xl="4" class="invoice-actions">
                    <b-card>
                        ...
                        <b-button @click="update" v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="primary" class="mb-75"
                            block>
                            {{ $t('save') }}
                        </b-button>
...
export default {
    ...
    data() {
        return {
            ...
            minDate: 'today',
            status: [
                {label: this.$t('work_orders.status.process'), value: 'En proceso'},
                {label: this.$t('work_orders.status.sending'), value: 'Enviado'},
                {label: this.$t('work_orders.status.received'), value: 'Recibido'}                
            ]
        };
    },
    computed: {
    ...
    methods: {
        async donwloadWorkOrder() {
            try {
                ...
                link.href = url;
                link.setAttribute('download', `Orden-${this.work_order_id}.pdf`);
                link.setAttribute('download', `Orden-${this.workorder.id}-${this.workorder.patient.full_name}.pdf`);
                link.click();
        ...
        async getWorkOrder() {
            ...
            this.loading = false
            this.workorder = data.data //.map(item => ({...item, application_date: moment(item.application_date).format('d/m/Y')}))
        ...
        async update() {
            try {
                this.loading = true
                const { data } = await WorkOrderResource.update(this.work_order_id, this.workorder)
                this.loading = false
                if (data.success) {
                    this.success(data.message)
                    router.push({ name: 'apps-work-orders-list' })
                } else {
